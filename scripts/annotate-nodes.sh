#!/bin/bash
test -n "$5" && echo CLUSTER is "$5" || "echo CLUSTER is not set && exit"
zone1=$(echo $1 | tr -d '[:space:]')
zone2=$(echo $2 | tr -d '[:space:]')
sub1=$(echo $3 | tr -d '[:space:]')
sub2=$(echo $4 | tr -d '[:space:]')
CLUSTER=$(echo $5 | tr -d '[:space:]')
nodeGroupName=$(echo $6 | tr -d '[:space:]')
customsg=$(echo $7 | tr -d '[:space:]')
kubectl get crd
# get the SG's
...
sleep 10
comm=`printf "kubectl get nodes --selector='eks.amazonaws.com/nodegroup==${nodeGroupName}' | grep Read | wc -l"`
target=`eval $comm`
comm=`printf "kubectl get node --selector='eks.amazonaws.com/nodegroup==${nodeGroupName}' -o json"`
allnodes=`eval $comm`
...
echo $nn $nz
echo "kubectl annotate node ${nn} k8s.amazonaws.com/eniConfig=${nz}-pod-netconfig --overwrite=true"
kubectl annotate node ${nn} k8s.amazonaws.com/eniConfig=${nz}-pod-netconfig --overwrite=true
done
