...
resource "aws_eks_node_group" "cluster_2_node_group1" {
  cluster_name    = aws_eks_cluster.eks_cluster_2.name
  node_group_name = "cluster_2_node_group1"
  node_role_arn   = aws_iam_role.eks_node_group_iam_role.arn
  subnet_ids      = data.aws_subnet_ids.main_public_subnets.ids
  instance_types  = ["m5.large"]
  scaling_config {
  ...
  tags = {
    "eks/cluster-name"                            = aws_eks_cluster.eks_cluster_2.name
    "eks/nodegroup-name"                          = format("ng1-%s", aws_eks_cluster.eks_cluster_2.name)
    "eks/nodegroup-type"                          = "managed"
    "eksctl.cluster.k8s.io/v1alpha1/cluster-name" = aws_eks_cluster.eks_cluster_2.name
  }

  depends_on = [
    null_resource.cluster_2_enable_cni_pod_sg
  ]
}
...
resource "null_resource" "cluster_2_enable_cni_pod_sg" {
  triggers = {
    always_run = timestamp()
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<EOT
        cn1=$(echo ${aws_eks_cluster.eks_cluster_1.name} | tr -d '[:space:]')
        cn2=$(echo ${aws_eks_cluster.eks_cluster_2.name} | tr -d '[:space:]')
        sg=$(echo ${aws_security_group.eks_pod_sg.id} | tr -d '[:space:]')
        echo -e "\x1B[35mEnabling CNI config......\x1B[0m"
        ./scripts/create-sg-policy.sh $cn1 $cn2 $sg
     EOT
  }
}
...
output "eks_outputs" {
  value = {
    ...
    cluster2 = {
      cluster_name       = aws_eks_cluster.eks_cluster_2.name
      cluster_endpoint   = aws_eks_cluster.eks_cluster_2.endpoint
      kubeconfig-ca-data = aws_eks_cluster.eks_cluster_2.certificate_authority[0].data
    }
    nodes = {
      cluster1_nodegroup1_arn = aws_eks_node_group.cluster_1_node_group1.arn
      cluster2_nodegroup1_arn = aws_eks_node_group.cluster_2_node_group1.arn
    }
  }
}
